...
- 运行事实由 `RunSpec / NodeRun / TraceEvent / ArtifactManifest / GateDecision / CredentialCheckResult` 共同承载。
- 第一版不做账号系统、云同步、多租户和复杂后台调度。
- 第一版优先证明 `WorkflowSpec -> Validate/Dry-run -> RunSpec Snapshot -> NodeRun ->
...
flowchart TB
  ...
  Trace["Run & Event Store<br/>RunSpec Snapshot / NodeRun / TraceEvent / GateDecision"]
...
RunSpec / NodeRun / TraceEvent / ArtifactManifest / GateDecision /
...
flowchart LR
  ...
  Intent --> Router --> Spec --> Validate --> DryRun --> RunSpec --> NodeRun --> Queue --> Adapter --> Result --> Persist
  Persist --> Events
  Persist --> Artifacts
  Review --> Gate
  Gate --> Persist
...
sequenceDiagram
  ...
  participant STORE as Event / Run / Artifact Store
  ...
  ORCH->>STORE: persist TraceEvent / ArtifactManifest / NodeRun
  ORCH->>STORE: persist AuditEvent / GateDecision when required
...
| `provider` | 模型或平台能力来源。 |
| `operation_id` | 稳定业务操作 ID，重试时不变，用于防止真实副作用重复。 |
...
- Runtime Adapter 不直接写 TraceEvent、ArtifactManifest、NodeRun 或 GateDecision。
...
| NodeRun | `not_started / queued / running / paused / blocked / failed / cancelled / timed_out / aborted / skipped / done` | 描述节点执行尝试，不承载审核决策。 |
...
- `pending_review / approved / rejected` 不写入 NodeRun。
- `reviewing` 不作为 GateDecision 的持久状态。
- NodeRun `done` 只代表生成动作完成。
- 下游放行看 GateDecision 与 ArtifactManifest，不只看 Agent 或 NodeRun 状态。
- 非法状态跳转必须拒绝并记录审计事件。
...
| `GateDecision` | JSON/JSONL | 可选 | 记录审核门决策、审核人、时间和下游放行依据。 |
...
- `TraceEvent` 必须包含 `event_id`、`sequence`、`run_id`、`node_id`、
  `event_type`、`event_category`、`actor`、`timestamp` 和 `payload`。
- 同一 run 内事件必须有递增序号或可排序时间戳。
- AuditEvent 是 `event_category: audit` 的受保护 TraceEvent 子类型。
- 每个业务操作必须有稳定 `operation_id`；每次执行尝试使用唯一 `attempt_id`。
- 外部副作用必须记录 `provider_receipt`，并在执行前按 operation_id 查重。
- `ArtifactManifest` 必须记录 `artifact_spec_id`、`run_id`、`node_run_id`、
  `produced_by`、`source_event_id`、`hash/path/status`。
...
- 外部结果不明确时，AdapterResult 标记 `unknown`，NodeRun 进入 `aborted` 并设置
  `reconciliation_required: true`，禁止直接重试。
