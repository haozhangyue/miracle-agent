...
- 运行事实由 `RunSpec / NodeRun / TraceEvent / ArtifactManifest / GateDecision` 共同承载。
...
| UI 请求校验 | 写操作必须经过 origin、session token 或本地一次性 token 校验。 |
| Service 到 Workspace | 只读写 workspace allowlist 内的 spec、run、trace、artifact manifest。 |
...
| 危险操作 | 删除、覆盖、发布、真实外部副作用操作必须显式确认并写入 AuditEvent。 |
...
flowchart LR
  ...
  Intent --> Router --> Spec --> Validate --> DryRun --> RunSpec --> NodeRun --> Queue --> AgentRun
  AgentRun --> Events
  AgentRun --> Artifacts
  Review --> Gate
  Gate --> Events
...
sequenceDiagram
  ...
  ORCH->>ADP: execute NodeRun with strategy
...
| `provider` | 模型或平台能力来源。 |
| `idempotency_key` | `run_id + node_id + attempt`，用于避免重复副作用。 |
...
| Spec 与 Run 真相混写 | 运行状态和审核结果污染 stable 模板 | 明确 WorkflowSpec 是模板真相，运行事实进入 RunSpec、NodeRun、TraceEvent、ArtifactManifest、GateDecision。 |
| 本地服务边界过宽 | 文件、CLI、MCP、API 调用存在产品化风险 | 默认绑定 127.0.0.1，写操作校验，路径和工具 allowlist，危险操作写 AuditEvent。 |
| JSONL 缺少恢复语义 | crash 后无法判断继续、重试或完成 | 增加 event_id、幂等键、manifest、GateDecision 和重建规则。 |
