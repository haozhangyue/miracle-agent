...
- PNG 展示图提供中文优先、英文对照版本，作为评审和沟通用的视觉资产，不替代 Mermaid 和正文中的精确依赖关系。
...
- 第一版优先证明 `WorkflowSpec -> Validate/Dry-run -> RunSpec / WorkflowSnapshot -> NodeRun/Attempt ->
  Agent Health -> Artifact Board -> Gate Review -> Evolution` 的闭环。
- 根据 `15_架构方案评审意见.md`，本轮先修高返工成本边界，再继续产品信息架构和原型规划。

展示图总览：

![Miracle 系统总架构图中文版](assets/architecture/miracle-system-architecture-zh.png)

<details>
<summary>查看英文原图</summary>

![Miracle System Architecture](assets/architecture/miracle-system-architecture.png)

</details>

### 1.1 中英术语映射

图片中的类名、字段名和事件名继续保留英文，以便与 Spec、日志和后续代码一致；产品概念和说明文字使用中文。

| 英文术语 | 中文含义 | 使用边界 |
|---|---|---|
| `Control Center` | 控制中心 | 任务启动、Run 状态、错误、成本和审计入口。 |
| `Workflow Studio` | 工作流工作台 | 编辑 WorkflowSpec、DAG、无限画布和子工作流。 |
| `Agent Collaboration` | 多 Agent 协同 | 查看 Agent 职责、状态、等待关系和交接产物。 |
| `Assets & Review` | 产物与审核 | 管理 Artifact、Gate、版本和放行结果。 |
| `Configuration Plane` | 配置平面 | 保存可版本化 Spec、Registry 和 ProviderPolicy。 |
| `Runtime Plane` | 运行平面 | 保存 RunSpec、NodeRun、NodeAttempt 和运行事实。 |
| `WorkflowSpec` | 工作流规格 | 模板与执行依赖的配置真相。 |
| `RunSpec` | 运行根对象 | 启动时冻结工作流版本、WorkflowSnapshot、组件和 ProviderPolicy。 |
| `NodeRun` | 节点运行实例 | 每个 run/node 的固定聚合实例。 |
| `NodeAttempt` | 节点执行尝试 | 每次真实外部调用对应一个不可变 Attempt。 |
| `operation` / `business revision` | 业务操作 / 业务修订 | 区分网络重试与审核返工、输入变化、用户重跑。 |
| `Runtime Adapter` | 运行时适配器 | 统一 Codex、Hermes、OpenClaw、MCP 和官方 API。 |
| `Provider` | 能力提供方 | 实际模型、平台、工具、渲染或外部 API。 |
| `Event Journal` | 权威事件日志 | 追加式记录运行事实，可用于重建投影。 |
| `Projections` | 可重建投影 | NodeRun、ArtifactManifest、Gate 等查询视图。 |
| `ArtifactSpec` | 产物契约 | 定义节点应生成什么，不代表某次真实产物。 |
| `ArtifactManifest` | 产物清单实例 | 记录某次运行的真实产物、路径、Hash 和状态。 |
| `GateInstance` | 审核门实例 | 某次 run 中绑定具体产物版本的审核过程。 |
| `GateDecision` | 审核决定 | 针对 GateInstance 的不可变最终决定。 |
| `AgentHealth` | Agent 健康状态 | 展示心跳、当前任务、等待对象、阻塞和恢复动作。 |
| `DISPATCHED` | 已派发 | 外部副作用调用已经发出，不能直接假定可重试。 |
| `RECEIVED` | 已收到结果 | 已获得完整、脱敏的 AdapterResult。 |
| `COMMITTED` | 投影已提交 | 运行事实已完成投影，不代表再次调用 Provider。 |
| `reconciling` | 对账中 | 外部结果未知，需要先向 Provider 核对。 |
| `review_policy` | 产物审核策略 | `none / auto / manual / conditional`。 |
| `join_policy` | 汇聚策略 | 定义必需输入就绪和可选分支等待行为。 |
| `required_inputs_ready` | 必需输入已就绪 | 必需入边满足后允许节点进入执行。 |
| `wait_if_active` | 已启动则有限等待 | 必须声明 max_wait、无合格产物终止时的动作和超时动作。 |
...
| 本地服务 | Node.js 本地服务 / Local Sidecar | Python 服务、纯 CLI | Node 与前端同栈，适合本地文件读写、CLI/MCP adapter、validate/dry-run 和本地 API；它不是商业化云端主后端的最终限定。 |
...
`Node.js 本地服务` 在本文件中指 Miracle MVP 的本地执行侧服务，也可以理解为
`Local Sidecar`，不是最终商业化云端主后端的唯一技术选型。

分层建议：

| 阶段 / 平面 | 推荐技术 | 职责 | 说明 |
|---|---|---|---|
| MVP 本地控制台 | React + Vite + TypeScript | Run 工作区、Attention、Artifact Board、Agent 视图 | 专注交互验证，不承担后端业务真相。 |
| MVP 本地服务 / Local Sidecar | Node.js / TypeScript | 本地文件读写、CLI/MCP 调用、HTTP adapter、validate/dry-run、状态流推送 | 与前端同栈，开发成本低，适合连接本地 workspace 和外部 runtime。 |
| 商业化云端控制平面 | Java/Kotlin + Spring Boot，或同等级强后端框架 | 用户、组织、多租户、权限、计费、Run 调度、审计、Registry、策略中心 | 适合复杂业务状态、事务一致性、并发调度和企业级治理。 |
| AI / 数据 / 工具 Worker | Python Worker，必要时补充 Node Worker | 文档解析、OCR、模型调用、数据处理、批处理、Provider 专用适配 | Python 生态适合 AI 和数据工具，但不建议作为唯一主控制平面。 |
| 队列与任务调度 | 云端任务队列 + Worker Pool | 长任务排队、限流、重试、幂等、失败对账 | P3 详细设计时定义，MVP 不提前实现完整后台调度平台。 |
| 数据存储 | PostgreSQL + 对象存储 + Event Journal | 租户数据、运行索引、审计事件、产物引用 | 商业化后 JSONL 可作为本地/导出格式，不作为云端查询主存储。 |

演进原则：

- Node.js 保留在本地执行侧：负责贴近用户机器的文件、CLI、MCP、Keychain/env 引用和本地
  adapter 调用。
- 云端主后端不被 Node.js 绑定：商业化后应按并发、事务、权限、审计和团队治理要求重新
  选择 Java/Kotlin、Go 或同等级后端栈。
- Python 作为能力型 Worker：用于 AI、文档、数据和专用工具处理，而不是把所有业务状态、
  计费和租户权限都压到 Python 服务里。
- UI 不直接绕过服务层：无论本地还是云端，UI 都只调用受控 API，不直接写文件、调 provider
  或修改运行事实。
...
Browser UI
  -> Cloud API / Control Plane
      -> Auth / Tenant / Billing / Registry / Run Scheduling / Audit
      -> Queue
          -> Worker Pool
              -> Python AI Worker
              -> Node Runtime Adapter Worker

Browser UI
  -> Local Node Sidecar
      -> Local Workspace / CLI / MCP / Keychain or env reference
      -> External Runtime
```

例如团队版用户启动一个“生成并发布日报”的 Run：

1. 云端 Java/Kotlin 控制平面负责校验租户权限、套餐额度、工作流版本、审核策略和审计记录。
2. 云端队列负责把长任务分发给 Worker，并做限流、重试和幂等控制。
3. Python Worker 负责 OCR、文档解析、模型辅助分析等 AI/数据处理。
4. Node Runtime Adapter Worker 或本地 Node Sidecar 负责调用 CLI、MCP、Codex、OpenClaw
   等更贴近工具生态的 runtime。
5. Event Journal、Run 投影、账单用量和审核决定由云端控制平面统一落库，避免多个 Worker
   直接写业务真相。
...
flowchart LR
  ...
  RunSpec["RunSpec<br/>workflow version / hash / resolved policy"]
  NodeRun["NodeRun"]
...
![Miracle 工作流生命周期图中文版](assets/architecture/miracle-workflow-lifecycle-zh.png)

<details>
<summary>查看英文原图</summary>

![Miracle Workflow Lifecycle](assets/architecture/miracle-workflow-lifecycle.png)
...
![Miracle 多 Agent 协同图中文版](assets/architecture/miracle-agent-collaboration-zh.png)

<details>
<summary>查看英文原图</summary>

![Miracle Multi-Agent Collaboration](assets/architecture/miracle-agent-collaboration.png)
...
![Miracle 产品能力架构图中文版](assets/architecture/miracle-product-architecture-zh.png)

<details>
<summary>查看英文原图</summary>

![Miracle Product Architecture](assets/architecture/miracle-product-architecture.png)
...
![Miracle 运维运行态架构图中文版](assets/architecture/miracle-ops-architecture-zh.png)

<details>
<summary>查看英文原图</summary>

![Miracle Ops Architecture](assets/architecture/miracle-ops-architecture.png)

</details>

图片生成源位于 `assets/architecture/source/generate-zh-diagrams.mjs`。中文版采用确定性 SVG 排版后渲染为 PNG，确保节点文字、箭头关系和评审结论可以稳定复现。
...
- GateSpec `actions` 使用 approve/reject 等命令词；GateDecision 使用
  approved/rejected 等结果词，二者不得混用。
- 非法状态跳转必须拒绝并记录审计事件。
...
- Flow A-G 中 B -> G 是 md_master required edge，F -> G 是 final_video optional edge；
  G 以 required_inputs_ready 启动。视频分支未启动时不等待；已启动时最多等待
  30 分钟；分支无合格产物终止时继续纯 MD 分发；超时要求用户决定。
- GateSpec.required_before 只约束 required edge 下游；F -> G 是 optional edge，因此
  F_render_gate 不无条件阻塞 G。
- ArtifactSpec.review_policy 是审核绑定真相；NodeSpec.review_gate 仅是便捷引用，
  两者不一致时 validate 失败。
...
| 自动化边界不可控 | AI 生成的新流程可能污染稳定模板，Auto Run 可能越过高风险节点 | 新流程只生成 draft/experimental；Auto 执行策略不得越过高风险 Gate。 |
| Spec 与 Run 真相混写 | 运行状态和审核结果污染 stable 模板 | 明确 WorkflowSpec 是模板真相，运行事实进入 RunSpec、WorkflowSnapshot、NodeRun、TraceEvent、ArtifactManifest、GateDecision、CredentialCheckResult。 |
...
| 可选视频边阻塞分发 | 纯 Markdown 流程无法进入 G | B -> G required、F -> G optional；Gate required_before 不约束 optional edge，并声明有限等待和终止策略。 |
...
| 把本地 Node Service 当作最终商业后端 | 后续多租户、权限、计费、并发调度和审计能力补齐成本升高 | 明确 Node.js 是 MVP Local Sidecar；商业化云端控制平面在 P3 重新评估 Java/Kotlin、Go 或同等级强后端栈。 |
