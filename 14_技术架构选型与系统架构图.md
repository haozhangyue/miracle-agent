...
flowchart LR
  ...
  Intent --> Router --> Spec --> Validate --> DryRun --> RunSpec --> NodeRun --> Queue --> Attempt --> Adapter --> Result --> Persist
  Persist --> Events
  Persist --> Artifacts
  Review --> Gate
  Gate --> Persist
...
sequenceDiagram
  ...
  ORCH->>JOURNAL: append adapter_result_received
...
| GateInstance | `pending_review / decided / blocked / skipped / invalidated` | 某次 run 中绑定具体产物版本的审核实例。 |
...
- AdapterResult 先追加 `adapter_result_received`，投影完成后追加
