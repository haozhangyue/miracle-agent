...
erDiagram
  ...
  NodeRun ||--o{ ArtifactManifest : produces
  ArtifactManifest }o--|| ArtifactSpec : realizes
  RunSpec ||--o{ GateDecision : records
...
## 10. RunSpec、WorkflowSnapshot 与 NodeRun
...
NodeRun 是单个节点的一次执行尝试记录，不承载产物审核决策。

```yaml
node_run_id: D_tts_caption_attempt_2
...
其中 `done` 只表示节点执行动作完成。产物能否被下游读取，由
`ArtifactManifest.status` 和 `GateDecision.decision` 决定。

副作用与重试规则：

- `operation_id` 是稳定业务操作 ID，重试时不变。
- `attempt_id` 是单次执行尝试 ID，每次重试都变化。
- 发布、删除、覆盖和付费生成必须记录 provider receipt。
- AdapterResult 不明确时，NodeRun 进入 `aborted` 并设置
  `reconciliation_required: true`，完成 reconcile 前不得重试。
...
ArtifactManifest 是某次 run 的真实产物，回答“这次任务实际生产了什么”。
...
GateDecision.decision = pending_review
```

下游放行必须显式引用 GateDecision 或 ArtifactManifest，不能使用含义不明的
`status in [...]`。
