...
| D3 | Gate reject 返工模型 | 可与 scheduler 设计并行 | Agent C | rework attempt、新 Artifact version、恢复规则 |
| D4 | Gate reject 返工 UI 与事件审计 | 依赖 D3 | Agent C | 返工动作、receipt、TraceEvent |
| D5 | 最小 scheduler 设计 | 可与 Adapter 目录并行 | Agent D | queued 节点扫描、Gate 人审暂停、operation lock 复用 |
| D6 | 最小 scheduler 执行闭环 | 依赖 D5 | Agent D | 自动执行 queued 节点、失败进入 Attention |
