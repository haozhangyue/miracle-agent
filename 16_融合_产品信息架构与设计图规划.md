...
- 状态标签显示对象归属，例如 `ArtifactManifest · pending_review`。
...
| Run 页面直接编辑 Workflow | 会混淆 WorkflowSnapshot 与 WorkflowSpec | Run 只读；MVP 只允许从快照创建 Workflow draft |
...
flowchart TB
  ...
  App --> Tasks["任务运行<br/>Run 列表 / Run 工作区 / Attention"]
...
| 任务运行 | 哪些 Run 在执行，如何继续或恢复 | RunSpec、WorkflowSnapshot、TraceEvent |
| 工作流 | 如何创建、编辑、验证和版本化自己的流程 | WorkflowSpec、NodeSpec、GateSpec |
| 智能体 | 谁在执行，能力和健康如何 | AgentSpec、AgentHealth、PermissionMatrix |
| 审核与交付 | 哪些产物需要审核，如何交付 | ArtifactManifest、GateInstance、发布包 |
| 资源库 | 如何发现、安装和升级可复用资产 | Registry、ComponentLibrary、ProviderPolicy |
...
两个工作域都可能显示 Workflow，但职责不同：

| 工作域 | 负责内容 | 不负责 |
|---|---|---|
| 工作流 | 创建、编辑、Validate、Dry-run、运行和版本管理用户自己的 Workflow | 外部资产发现和安装 |
| 资源库 | 发现、安装、升级、弃用外部或内置的 Workflow、组件、Agent 模板和策略 | 直接编辑用户运行中的 Workflow |
| 智能体 | 管理实际 AgentSpec、健康、装备、权限和运行情况 | 把 Agent 模板当成运行 Agent |

发布关系：

```text
用户工作流
-> Validate / Dry-run / Sample Run
-> 从工作流详情发布为 Registry 资产
-> 资源库负责发现、安装、更新和弃用
```

资源库中的 Agent 模板是可安装定义，不代表正在运行的 Agent 实例。
...
Attention Queue 使用“根因聚合 + Run 级行动项”：

```text
Root Cause / Incident
├── Attention Item: Run A
├── Attention Item: Run B
└── Attention Item: Run C
```

规则：

- 使用稳定的 `root_cause_id` 或 `attention_key` 识别同一根因。
- 同一对象、同一根因、同一恢复动作只保留一个活动 Item。
- 同一凭证或 Provider 问题影响多个 Run 时，可以聚合展示，但每个 Run 保留独立行动项。
- NodeRun、AgentHealth、ArtifactManifest 和 GateInstance 作为关联对象展开，不重复制造
  含义相同的顶层卡片。
- 一个根因存在不同影响、优先级或恢复动作时，不强行合并为一张卡。
...
```text
open -> acknowledged / snoozed -> resolved -> history
```

约束：

- 底层权威状态恢复后自动进入 `resolved`，用户不能手动伪造 resolved。
- resolved 项进入历史记录，不直接删除。
- 低风险提醒允许 acknowledge 或 snooze。
- 未完成审核、reconciling 和危险操作确认不允许关闭或隐藏。
- snooze 只延后提醒，不改变底层状态。
...
| 选择方式 | 默认用户 | 行为 |
|---|---|---|
| 推荐模板 | 大多数用户 | 推荐 stable 工作流和默认组件 |
| 从模板自定义 | 熟悉流程的用户 | 从模板创建草案并调整参数和策略 |
| AI 生成草案 | 需要系统辅助规划的用户 | 生成 draft Workflow，确认后才能启动 |

AI 生成草案不得自动发布 stable，也不得越过人工审核门。

工作流选择方式与执行策略是两个独立维度。执行策略继续沿用架构定义：

```text
Auto / Manual / Hybrid
```

- `Auto`：自动推进低风险节点，高风险节点仍保留 Gate。
- `Manual`：用户明确控制工作流、Agent、组件和 Provider。
- `Hybrid`：系统推荐并自动推进部分环节，用户确认关键决策。
...
```mermaid
flowchart TD
  Start["输入目标、素材和输出要求"] --> Select{"推荐模板 / 从模板自定义 / AI 生成草案"}
  Select --> Workflow["确认 Workflow 与任务变量"]
  Workflow --> Strategy["选择执行策略<br/>Auto / Manual / Hybrid"]
...
左侧：从 WorkflowSnapshot 动态生成的流程大纲 / 阶段过滤器
中间：当前主视图
右侧：对象详情、影响范围和恢复动作
底部：事件、审计和系统消息抽屉
```
...
六个固定视图是页面导航；流程大纲不是第二套页面导航：

- 默认包含“全部阶段”。
- 点击阶段只过滤或聚焦当前视图中的节点、Agent、产物和事件。
- 点击阶段不切换固定视图。
- 阶段由 WorkflowSnapshot 的显式分组、子工作流或 DAG 结构动态生成。
- 通用产品规范不硬编码 Flow A-G。
...
flowchart LR
  ...
  Run --> Draft["从快照创建 Workflow draft"]
...
新 draft 用于未来 Run，不修改当前 Run 的 WorkflowSnapshot。`Run patch` 在其字段范围、
快照语义、已完成节点、Gate、Artifact 和 resolved inputs 迁移协议由 P3 明确定义前，
不进入 P2/MVP 产品界面。
...
AgentHealth · waiting
AgentHealth · reviewing
NodeRun · failed
NodeAttempt · failed
NodeAttempt · timed_out
NodeRun · reconciling
```
...
| 对象状态 | 必须显示 | 操作限制 |
|---|---|---|
| `AgentHealth.waiting` | 等待对象 | 不显示无意义重试 |
| `NodeRun.blocked` | 原因、影响和恢复动作 | 修复后重新调度 |
| `NodeRun.failed` | 聚合失败原因和可恢复条件 | 不能绕过修复生成 approved |
| `NodeAttempt.failed` | 本次尝试错误和 retry/fallback 条件 | 新 Attempt 保留旧历史 |
| `NodeAttempt.timed_out` | Attempt 和外部状态 | 先判断是否可安全重试 |
| `NodeRun.reconciling` | receipt、核对动作和重复副作用风险 | 禁止普通重试 |
...
- 整体健康状态和可用容量摘要。
- 活跃执行数量、排队数量和阻塞数量。
- 当前执行列表，每项显示 Run、NodeRun、状态和开始时间。
- 排队任务和阻塞任务。
...
P2 只定义聚合展示需求，不把 `AgentHealth.current_run/current_node` 直接改成数组。P3 需要
明确 AgentSpec、runtime/session、执行实例、并发容量和健康投影之间的身份关系。
...
```text
Run / ArtifactManifest ID / version / hash / GateInstance / 发布目标
```

从产物进入审核、再进入交付时必须保持：

- 当前 Run。
- 当前 ArtifactManifest 版本和 hash。
- GateInstance 状态。
- 发布目标。
- 当前筛选条件。
- 返回原 Run 的入口。
...
```mermaid
flowchart TD
  PendingArtifact["ArtifactManifest · pending_review"] --> PendingGate["GateInstance · pending_review"]
  PendingGate --> Action{"审核动作"}
  Action -->|approve| Approved["GateDecision · approved"]
  Approved --> Release["ArtifactManifest · approved<br/>允许下游读取"]
...
用户自己的 Workflow 在“工作流”中编辑和版本管理；发布为 Registry 资产后，才进入
资源库的发现、安装、升级和弃用生命周期。
...
| `P2F-02` | 新任务-描述与推荐 | 推荐模板/从模板自定义/AI 生成草案 |
| `P2F-03` | Validate / Dry-run | 缺凭证、Gate、成本、风险 |
| `P2F-04` | Run 工作区 | 总览/流程切换、动态阶段过滤、只读快照 |
| `P2F-05` | Attention Queue | review、blocked、reconciling、conflict |
| `P2F-06` | 审核抽屉 | ArtifactManifest、GateInstance、版本和返工 |
| `P2F-07` | Agent Collaboration | Agent Map、并发执行、等待和交接 |
| `P2F-08` | 工作流 DAG 编辑态 | NodeSpec、EdgeSpec、Diff |
| `P2F-09` | Infinite Canvas | 七个区域和任务卡转节点 |
| `P2F-10` | 智能体中心 | 跨 Run 健康、容量、装备和 Provider |
| `P2F-11` | 审核与交付 | 产物、审核、交付和上下文连续性 |
| `P2F-12` | 资源库 | 可发现和可安装的模板、组件、Agent 和策略 |
...
P2F-02 新任务
P2F-03 Dry-run
P2F-04 Run 工作区（总览/流程）
P2F-05 Attention Queue
P2F-06 审核抽屉
```

Agent Collaboration 作为独立任务测试；工作流编辑、Canvas、智能体中心、完整审核与
交付和资源库在主任务闭环通过后补齐。
...
| T9 | 查看多 Agent 协同 | 识别并发执行、排队、阻塞和交接产物 |
...
两者都从 WorkflowSnapshot 动态生成，只过滤或聚焦当前视图，不改变 Run 的 6 个固定
视图。
...
第一轮不把 Agent Collaboration 绑定到审核返工链路中，另用独立任务验证并发执行、
等待关系和产物交接。
...
- Attention 数量变化和关键状态变化提供可感知的文本反馈。
- 非紧急更新使用礼貌级实时通知，避免重复播报。
- 实时刷新不得抢走键盘焦点或重置用户当前选择。
- NodeRun 从 running 变为 blocked/failed/reconciling 时，列表替代视图同步更新。
- 超时提醒是否可延长、snooze 或必须立即决定，由 workflow policy 决定。
- 未完成审核、reconciling 和高风险确认不得通过无障碍设置永久隐藏。
...
| F11 | 原型是否桌面优先 | 是 |
| F12 | 是否先低保真再高保真 | 是 |
| F13 | 工作流选择与执行策略是否分离 | 是 |
| F14 | Run patch 是否进入 P2/MVP | 否 |
| F15 | Attention 是否有去重和生命周期规则 | 是 |
| F16 | Run 阶段是否作为第二套页面导航 | 否，只是动态过滤器 |
| F17 | Agent 并发协议是否在 P2 定型 | 否，P2 定展示需求，P3 定协议 |

## 29. 后续执行顺序

1. 确认 F01-F17 产品决策。
2. 使用 Pencil 建立共享组件。
3. 制作 `P2F-01` 至 `P2F-06`。
4. 串联 T1、T2、T3、T6 和 T10。
5. 完成第一轮 3-5 人快速测试。
6. 修正首页、Attention、Run 和审核交互。
7. 独立验证 T9 多 Agent 并发协同。
8. 补齐 `P2F-07` 至 `P2F-12`。
9. 验证 DAG、Canvas、Spec 和 Workflow draft 流程。
10. 完成第二轮任务可用性测试。
11. P2 评审通过后形成高保真规范，并发布 `v0.6.0`。
