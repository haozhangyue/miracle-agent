...
- 已确认角色化说明书与 Web 帮助中心设计：后续将现有 `40` 收敛为统一入口，并新增
  使用者、管理员、开发维护者、故障排查和用户可感知版本变化五类手册。
- Web 计划新增“帮助与手册”一级入口，通过受控 Help Manifest 和 Sidecar 只读 API
  使用仓库中的同一套 Markdown，不复制第二份帮助文案。
- 本次只固化设计基线，尚未上线新菜单、Help API 或 `61-65` 正式手册，当前用户操作
  真相仍为 `40_Miracle系统操作使用说明书.md`。
