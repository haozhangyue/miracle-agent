...
| 当前阶段 | P7-01 至 P7-10 已完成；角色化说明书与 Web 帮助中心已交付 |
...
- 完成 `61-65` 角色化说明书：使用者、管理员、开发维护者、故障排查和用户可感知
  版本变化各有独立阅读路径；`40` 收敛为兼容总入口。
- Web 新增“帮助与手册”一级入口，支持角色筛选、全文搜索、本页目录、版本标记、
  Markdown/GFM、Mermaid、受控图片和 `page=help&article=...#anchor` 深链接。
- Sidecar 新增只读 Help API：帮助索引、文章、搜索和 allowlist 图片；Manifest 不向
  Web 暴露源路径，路径穿越、绝对路径和越界 symlink 会被拒绝。
- 新增 `assets/manual/v0.9.0/` 的 12 张当前界面截图，并通过 Playwright 验证首页、
  新任务、Dry-run、Run、Attention、Agent、Artifact、Gate、Canvas、Task Baseline
  和帮助中心；浏览器控制台无错误或警告。
- 新增 H1-H7 实施计划与交付验收报告，同步 README、AI 阅读导航和 task-baseline。
...
- 模型 API 采用 `ModelApiAdapter -> ProviderDriver -> ProviderProfile` 分层，预定接入
  DeepSeek、Kimi 和 MiniMax。
- P7 明确不接入 OpenAI 官方 API，Hermes/OpenClaw 仅保留契约兼容性。
- P7-01 已通过人工评审，新增 `56_P7工程实施计划与任务拆解.md`，把 P7-02 至
...
- P7-05 最终兼容修复：Gate 驳回与独立 P0 blocker 并存时，Scheduler 优先返回
...
- 新增 `docs/README.md` 作为目录索引，补充每个领域和子目录的中文名称、概念设计、
  设计原因、实际路径和 AI 阅读入口。
- 同步根 `README.md`、`docs/00-navigation/asset-index/17_文档资产关联与AI阅读导航.md`
  以及 task-baseline 证据路径，避免目录迁移后出现失效链接或误读根目录旧路径。
- 本次为文档治理和路径整理，不改变运行时代码、工作流协议、task-baseline 任务状态、
  本地启动命令或 Web 菜单操作；验证完成后再形成正式版本收口记录。
