...
| `failed` | 执行失败，需要修复。 |
| `cancelled` | 用户或系统取消，是当前 attempt 的终态。 |
| `timed_out` | 超过执行时限，是否重试由 failure policy 决定。 |
| `aborted` | 进程被强制终止，必须 reconcile 后再决定重试。 |
| `skipped` | 本轮跳过。 |
...
| 对象 | 状态或决策 |
|---|---|
| NodeRun | `not_started / queued / running / paused / blocked / failed / cancelled / timed_out / aborted / skipped / done` |
...
`cancelled` 恢复时创建新的 attempt；`aborted` 和外部副作用结果不明时进入
reconcile，不能直接重试。
