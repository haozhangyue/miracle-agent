...
| GateInstance | `pending_review / decided / blocked / skipped / invalidated` |
...
重试、fallback、审核返工和重新执行只新增 NodeAttempt，不新增 NodeRun 或 DAG
节点。NodeAttempt 为 `unknown` 时，NodeRun 进入 `reconciling`，核对完成前不能
直接执行下一次真实副作用。
