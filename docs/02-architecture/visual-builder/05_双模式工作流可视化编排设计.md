...
- `WorkflowSpec / AgentSpec / ComponentSpec` 是可版本化配置真相，UI 和配置文件都只是编辑器。
- 运行事实由 `RunSpec / WorkflowSnapshot / NodeRun / NodeAttempt / TraceEvent /
  ArtifactManifest / GateInstance / GateDecision / CredentialCheckResult` 承载，
  不直接写回 stable 工作流模板。
...
- 任务卡转为执行节点后必须生成 NodeSpec。
- Agent 卡参与执行时必须绑定 AgentSpec 或 AgentRun。
- 模板设计态的产物占位卡绑定 ArtifactSpec。
- 运行态产物卡绑定 ArtifactManifest，并反向显示 ArtifactSpec。
- 灵感卡可以只作为 CanvasNote 存在，不进入执行。
...
6. 生成 spec diff，并提示是否需要 dry-run。
...
### 6.4 Visual / Spec 双向同步

同步规则：

- UI 新增节点必须生成 NodeSpec diff。
- YAML/JSON 新增节点必须反映到 DAG 和 Canvas。
- UI 修改 layout 只影响 `layouts`，不影响 `edges`。
- UI 删除卡片时必须区分“隐藏卡片”和“删除 NodeSpec”。
- stable 模板被修改时默认创建 local override，不直接覆盖 stable。
...
| Registry 远端模板有新版本 | 显示 changelog 和 breaking changes。 |
| layout 引用不存在节点 | validate warning，可自动清理 layout。 |
...
| Spec 同步 | UI 新增节点能生成 spec diff，YAML 新增节点能回显到 UI。 |
| dry-run | 流程节点模式能显示凭证、审核门、provider 和风险预览。 |
