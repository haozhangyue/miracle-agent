...
- 本节示例中的 `current_node` 表示单个运行投影的当前执行，不代表 AgentSpec 全局只能
  参与一个 Run。
- 产品层可以按 AgentSpec 聚合多个活跃、排队和阻塞的运行投影。
- P2 展示整体健康、活跃执行数、排队数、阻塞数和执行列表。
- P3 再确定 AgentSpec、runtime/session、执行实例、并发容量和 AgentHealth 投影的
  具体身份键；MVP 不把 `current_run/current_node` 直接机械改成数组。
...
运行事实写入不属于 Agent 权限；`TraceEvent / NodeRun / ArtifactManifest /
GateDecision` 只能由 Orchestrator 根据 AdapterResult 或用户审核动作单写入。
