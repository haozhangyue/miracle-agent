...
> 文档状态：已落地目录索引。
>
> 本文件定义 Miracle 文档的目录、中文显示名称、文档职责和迁移映射。00-55 文档已按
...
2. 原有 `00-55` 编号保留在文件名中，解决阶段顺序和 Git 历史追溯问题。
...
| 原文件 | 目标目录（中文对照） | 状态 | 概念设计与原因 |
|---|---|---|---|
| `55_P7多节点真实执行与模型Adapter扩展总体设计.md` | `05-delivery/p7-adapter-expansion/`（交付/P7 Adapter 扩展） | CURRENT | 先定义 Codex 多节点纵向闭环、Artifact 交接和 retry/fallback，再以通用 Model API Adapter 接入 DeepSeek、Kimi 和 MiniMax，避免工作流绑定厂商。 |
...
2. 保留 `00-55` 编号和文件名，不用目录名称替代文档状态。
...
- 00-55 文档已全部进入 `docs/` 对应领域目录，历史评审和候选方案进入 `99-archive/`。
