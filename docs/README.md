...
> 文档状态：已落地目录索引。
>
> 本文件定义 Miracle 文档的目录、中文显示名称、文档职责和迁移映射。00-60 文档已按
...
│   └── p7-adapter-expansion（P7 多运行时与 Adapter 扩展）/
├── 06-operations（操作、测试与发布）/
│   ├── user-guide（用户操作手册）/
│   ├── manuals（角色化说明书与 Web 帮助中心）/
│   ├── testing（测试与回归）/
│   └── release（版本与阶段收口）/
├── 90-reference（参考资料）/
...
| `06-operations/manuals`（角色化说明书与 Web 帮助中心） | 按使用者、管理员、开发维护者组织操作资料，并向 Web 提供同源帮助内容 | 角色分册降低阅读成本，Help Manifest 保证仓库文档与应用内帮助不形成两套真相 |
...
| `54_P6回归验收与版本收口报告.md` | `06-operations/release/`（运维/P6 版本收口） | ACTIVE | 记录 P6 工程、API、页面、安全真实性验收和 `v0.8.0` 发布结论。 |
...
| `56_P7工程实施计划与任务拆解.md` | `05-delivery/p7-adapter-expansion/`（交付/P7 实施计划） | ACTIVE | P7-02 至 P7-10 的逐文件 TDD 实施计划与全部完成记录。 |
| `57_P7-07模型Provider接入交付说明.md` | `05-delivery/p7-adapter-expansion/`（交付/P7 Provider） | ACTIVE | 记录三家 Provider Driver、Profile、Provider API、安全边界与 DeepSeek 真实 smoke；Kimi/MiniMax 仍为 `configured_unverified`。 |
| `58_P7-08Provider路由与Fallback交付说明.md` | `05-delivery/p7-adapter-expansion/`（交付/P7 路由） | ACTIVE | 记录确定性 Provider Router、同类 fallback、跨 kind 人工确认、Run 级决策审计和未验证 Provider 禁止执行边界。 |
| `59_P7-09多运行时UI与可观测性交付说明.md` | `05-delivery/p7-adapter-expansion/`（交付/P7 可观测性） | ACTIVE | 记录 Run/Attention/Artifact 的只读观测投影、retry 停止审计和跨 kind fallback 二次确认。 |
| `60_P7回归验收与版本收口报告.md` | `06-operations/release/`（运维/P7 版本收口） | CURRENT | P7-10 最终真相源：全部验收通过，DeepSeek 真实 smoke 成功并发布 `v0.9.0`。 |
