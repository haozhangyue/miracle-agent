...
> 文档状态：`ACTIVE / P7-02 至 P7-10 已完成实施基线`
>
...
未勾选说明：前任执行过该初始 RED，但未保留完整原始命令日志，无法独立复核“view-model
未实现”的失败现场。本次收尾另有成本范围与历史 Run 动作投影的可复现 RED，详见 Task 8 report。

- [x] **Step 3: 实现 Run 可观测投影**

Node Detail 展示 runtime、provider profile、model、operation/attempt 关系、retry 倒计时、
fallback 原因、usage、估算/实际成本。状态必须继续带对象归属。

- [x] **Step 4: 实现 Attention 恢复操作**

支持查看 retry budget、停止自动重试、确认 fallback、打开凭证配置说明和返回 Run。
未实现的恢复操作不显示可点击按钮。

- [x] **Step 5: 保持 Product Design A/B/C 视觉基线**
...
**Files:**
- Create: `docs/06-operations/release/60_P7回归验收与版本收口报告.md`
...
2026-08-01 经用户明确授权，通过一次性进程环境完成 DeepSeek `deepseek-v4-flash` 脱敏
completion smoke；300 tokens、2884 ms，仓库外 Artifact 和脱敏摘要已核对。Kimi、MiniMax
未真实验证，默认 fixture 仍保持 `configured_unverified`，不把一次本机验证固化成全局健康。

- [x] **Step 5: 验收 Web 并生成截图**
...
- [x] **Step 7: 更新 task-baseline 并提交**

`p7-10 -> completed`，P7 phase -> completed，下一节点只在 60 号报告审批后设为 current。

60 号报告最终结论为全部通过；`p7-10`、P7 与 L3 标记为 `completed`，发布 `v0.9.0`。
下一阶段尚未规划，`current_node_id` 置空，不创建 P8 占位任务。

Commit: `git commit -m "记录P7回归验收与Provider发布门"`
