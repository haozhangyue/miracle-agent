...
- Produces: `buildRunExecutionPlan(runId)`, Scheduler response field `execution_plan`.
...
`decision === "execute"` 的 NodeRun。不再用“queued 即可执行”作为唯一条件。
...
Orchestrator 在 commit tick 开始时写 `execution_plan_calculated`，在每个节点启动前写
`node_inputs_resolved`。事件 message 只记录 ID、数量和 reason code，不记录 Artifact 全文。
...
expect(attempts(await getRun(runId), "C_md_master")).toHaveLength(2);
```
...
- [ ] **Step 3: 实现 RetryPolicy 和 RetryDecision**
...
- [ ] **Step 4: 实现原子 RetryScheduleRecord**
...
0 命中。未实测的其他 Provider 保持 `configured_unverified`。
...
- [ ] **Step 1: 写成本/能力/健康和跨 kind 确认失败测试**
...
确认体包含 operation ID、expected current adapter kind、target profile ID 和 actor，防止过期确认覆盖
新决策。
