...
- P6-08 已通过验收并发布 `v0.8.0`；本文保留为 P6 实施与验收追溯基线。
...
- [x] **Step 2: 连接 RunDraft 与唯一 startRun 服务**
...
- [x] **Step 3: 实现 JSONL parser 和输出校验**
...
- [x] **Step 8: 同步操作手册、截图、task-baseline 并提交**
...
| P6-06 | Codex CLI Health 与 Attempt Workspace | completed | P6-05 | 是 | health、隔离 workspace、进程控制 |
| P6-07 | C_md_master 单节点真实执行 | completed | P6-03/P6-04/P6-06 | 否 | 正式 Run、Markdown、Artifact、Gate、Trace |
| P6-08 | 回归验收与版本收口 | completed | P6-07 | 否 | 回归报告、截图、`v0.8.0` 版本决策 |
...
`P6-04` 至 `P6-08` 已完成并通过回归，发布 `v0.8.0` 真实工作流工程接入基线。
当前进入 `P7-01` 多节点真实执行与 Adapter 扩展规划。
