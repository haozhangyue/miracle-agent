...
`40_Miracle系统操作使用说明书.md` 是兼容总入口；当前操作真相由角色化分册共同组成：
`61` 使用者、`62` 管理员、`63` 开发维护、`64` 故障排查、`65` 用户可感知版本变化。
AI 应按当前任务只读取对应角色分册，避免为一次操作加载全部手册。

`00_Miracle角色化说明书与Web帮助中心设计.md` 是设计基线，`66` 是实施计划，`67` 是
交付验收结论。Web 左侧“帮助与手册”和仓库手册读取相同 Markdown；Help Manifest、
Sidecar 只读 API、搜索、Mermaid、受控图片与深链接均已上线并通过 `v0.9.0` 验收。
...
`54_P6回归验收与版本收口报告.md` 是 `v0.8.0` 发布与 P6 最终验收基线。
...
基线；`56_P7工程实施计划与任务拆解.md` 是 `P7-02` 至 `P7-10` 的实施计划，
`57_P7-07模型Provider接入交付说明.md` 是 P7-07 的 Provider 接入基线，
`58_P7-08Provider路由与Fallback交付说明.md` 是路由交付说明，
`59_P7-09多运行时UI与可观测性交付说明.md` 是可观测性交付与 P7-10 验收输入，
`60_P7回归验收与版本收口报告.md` 是 P7 最终发布结论的唯一真相源。
当前 `P7-02` 至 `P7-10` 已全部完成，DeepSeek 真实 smoke 已通过并发布 `v0.9.0`；P7、
`p7-10` 与 L3 均为 `completed`，下一阶段待规划。P7-05 已覆盖显式 failed 错误分类、fixed/exponential
退避、attempt/time/cost 预算、原子 schedule、重启幂等恢复和根因 Attention，P7-06 已建立
通用 Model API Adapter、credential 授权、原生 fetch 传输边界和 operation 取消契约；
P7-07 已接入 DeepSeek/Kimi/MiniMax Driver、Profile 和 Provider API；DeepSeek 真实 smoke
已通过，Kimi/MiniMax 与默认 fixture 仍为 `configured_unverified`。P7-08 已完成确定性 Router、同类 fallback、
跨 kind 人工确认和 Run 级审计；P7-09 已完成 Run/Attention/Artifact 多运行时可观测投影、
受控 retry 停止和 fallback 二次确认。
它们共同确立 Codex 多节点纵向闭环优先、Provider fallback 后置、DeepSeek/Kimi/MiniMax
通过通用 Model API Adapter 接入，以及 P7 不接 OpenAI 官方 API 的边界。进入工程
验收和发布判断时优先读取 60；核对设计、计划与 UI 输入时读取 55、56、59，核对路由/fallback
时补读 58，维护 Provider Driver 时再读 57，
并按任务回读对应 P6 交付文档。
...
flowchart TD
  ...
  P40 --> H0["角色化说明书与 Web 帮助中心设计<br/>ACTIVE"]
  H0 --> H1["61-65 角色化手册 + Help API/Web<br/>CURRENT"]
  H1 --> H2["67 帮助中心交付验收<br/>CURRENT"]
  ...
  P701 --> P702["56 P7 实施计划<br/>ACTIVE"]
  P702 --> P707["57 P7-07 Provider 接入<br/>ACTIVE"]
  P707 --> P708["58 P7-08 Fallback/路由<br/>ACTIVE"]
  P708 --> P709["59 P7-09 UI/可观测<br/>COMPLETED"]
  P709 --> P710["60 P7-10 全部通过 / v0.9.0<br/>CURRENT"]
```
...
- `40_Miracle系统操作使用说明书.md` 是角色化手册兼容总入口；具体操作按 `61-65`
  分册读取，`67` 记录 Help API、Web 入口和截图验收结论。
...
- `60_P7回归验收与版本收口报告.md` 是 P7-10 最终发布真相源，记录工程、真实 Codex、
  fake 异常路径、DeepSeek 真实 smoke、`v0.9.0` 发布和 P7 收口决策。
...
| `61_Miracle使用者操作手册.md` | `CURRENT` | 40、当前 Web/API、P7 验收基线 | Web 帮助中心、用户验收 | 创建任务、Run、Attention、Artifact、Gate 与工作流操作时读 |
| `62_Miracle管理员与运维手册.md` | `CURRENT` | 40、Sidecar、Provider/Adapter、P7 验收基线 | 部署运维、凭证和备份 | 安装、启动、Provider、数据、安全任务时读 |
| `63_Miracle开发维护手册.md` | `CURRENT` | 19-23、当前工程、P7 协议 | 工程扩展与发布 | 修改 Workflow、Adapter、Provider、API、Web 或发布流程时读 |
| `64_Miracle故障排查手册.md` | `CURRENT` | 61-63、Attention、Retry/Fallback | 运行恢复 | blocked/failed/Gate/Artifact/Provider 故障时读 |
| `65_Miracle用户可感知版本变更.md` | `CURRENT` | VERSION_HISTORY、各阶段发布报告 | 用户升级感知 | 查询当前版本相对历史版本变化时读 |
| `66_Miracle角色化说明书与Web帮助中心实施计划.md` | `ACTIVE` | 00 设计、40、当前工程 | 61-67、task-baseline | 追溯 H1-H7 实施边界时读 |
| `67_角色化说明书与Web帮助中心交付验收报告.md` | `CURRENT` | 61-66、Help API/Web、截图与回归 | 后续说明书迭代 | 判断帮助中心是否完成和如何维护时读 |
...
| `54_P6回归验收与版本收口报告.md` | `ACTIVE` | 47-53、工程回归、真实 W24/W23、页面截图 | P7-01、60 | `v0.8.0` 发布与 P6 结论追溯时读 |
| `55_P7多节点真实执行与模型Adapter扩展总体设计.md` | `ACTIVE` | 54、01、03、14、45、51-53 | 56、P7-02 至 P7-10 | 已评审的 P7 纵向闭环、retry/fallback 和模型 API 分层基线 |
| `56_P7工程实施计划与任务拆解.md` | `ACTIVE` | 55、54、51-53、Core、Sidecar、Web | 57-60 | P7 已完成的逐文件 TDD 实施计划、依赖、提交点和验收命令追溯 |
| `57_P7-07模型Provider接入交付说明.md` | `ACTIVE` | 55、56、Core、Sidecar、Provider Profile | 58、Provider Driver 维护 | P7-07 工程接入、凭证状态、DeepSeek 真实 smoke 和安全边界必读 |
| `58_P7-08Provider路由与Fallback交付说明.md` | `ACTIVE` | 55-57、Core、Sidecar、Retry | 59、P7-10 | Provider Router、fallback、人工确认、路由审计和健康边界必读 |
| `59_P7-09多运行时UI与可观测性交付说明.md` | `ACTIVE` | 55-58、apps/web、apps/sidecar | 60 | Run/Attention/Artifact 观测投影、停止自动 retry、二次确认和截图证据维护时读 |
| `60_P7回归验收与版本收口报告.md` | `CURRENT` | 54-59、工程回归、真实 Codex、fake 异常路径、DeepSeek smoke、P7 截图 | 下一阶段规划 | P7 最终通过、`v0.9.0` 和 task-baseline 收口决策必读 |
...
### 7.5.1 P7 Provider 任务

优先读取：

```text
55_P7多节点真实执行与模型Adapter扩展总体设计.md
56_P7工程实施计划与任务拆解.md
57_P7-07模型Provider接入交付说明.md
58_P7-08Provider路由与Fallback交付说明.md
59_P7-09多运行时UI与可观测性交付说明.md
60_P7回归验收与版本收口报告.md
```

P7-10 与后续发布判断以 60 为唯一当前真相源；56 和 59 用于追溯验收范围与 UI 输入，58
用于核对路由/fallback。57 记录 DeepSeek 真实 smoke 及 Kimi/MiniMax 未验证边界；不得把
一次开发机验证固化为所有环境的 Provider healthy。
...
5. `docs/06-operations/manuals/` 对应角色分册、Help Manifest 和版本化截图：如果影响
   用户启动、菜单、操作路径、可感知功能或 bug 修复；总入口变化时同步更新 `40`。
