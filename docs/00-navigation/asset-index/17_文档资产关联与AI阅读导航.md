...
`59_P7-09多运行时UI与可观测性交付说明.md` 是当前可观测性交付与 P7-10 验收输入。
当前 `P7-02` 至 `P7-09` 已完成，`P7-10` 回归验收与版本收口为当前任务；P7-05 已覆盖显式 failed 错误分类、fixed/exponential
...
真实 smoke 未执行，状态仍为 `configured_unverified`。P7-08 已完成确定性 Router、同类 fallback、
...
验收时优先读取 55、56、59；核对路由/fallback 时补读 58，维护 Provider Driver 时再读 57，
并按任务回读对应 P6 交付文档。
...
flowchart TD
  ...
  P709 --> P710["P7-10 回归收口<br/>CURRENT"]
```
...
| `59_P7-09多运行时UI与可观测性交付说明.md` | `CURRENT` | 55-58、apps/web、apps/sidecar | P7-10 | Run/Attention/Artifact 观测投影、停止自动 retry、二次确认和截图证据必读 |
...
P7-10 验收先以 56 的验收范围和 59 的 P7-09 交付证据为入口，再用 58 核对路由/fallback；
57 记录当前三家凭证缺失和真实 smoke 未执行的事实，不得以 Profile 配置或 fake-server 验收推断 Provider healthy。
