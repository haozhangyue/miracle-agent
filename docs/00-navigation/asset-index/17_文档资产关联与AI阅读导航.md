...
`60_P7回归验收与版本收口报告.md` 是当前 P7 发布门的唯一真相源。
当前 `P7-02` 至 `P7-09` 已完成；`P7-10` 工程回归和真实 Codex 三节点闭环有条件通过，
但至少一个真实 Provider smoke 未完成，P7 和 `p7-10` 仍为 `current`。P7-05 已覆盖显式 failed 错误分类、fixed/exponential
...
P7-07 已接入 DeepSeek/Kimi/MiniMax Driver、Profile 和 Provider API，但三家凭证均缺失，
真实 smoke 未执行，状态仍为 `configured_unverified` 且不可执行。P7-08 已完成确定性 Router、同类 fallback、
...
flowchart TD
  ...
  P709 --> P710["60 P7-10 条件通过 / 发布阻塞<br/>CURRENT"]
```
...
- `60_P7回归验收与版本收口报告.md` 是 P7-10 当前发布门真相源，记录工程与真实 Codex
  闭环通过、fake 异常路径证据、真实 Provider smoke 缺失、`v0.8.0` 保持和 P7 不收口决策。
...
| `56_P7工程实施计划与任务拆解.md` | `ACTIVE` | 55、54、51-53、Core、Sidecar、Web | 57-59、P7-10 工程提交 | P7 当前逐文件 TDD 实施计划、依赖、提交点和验收命令必读 |
| `57_P7-07模型Provider接入交付说明.md` | `ACTIVE` | 55、56、Core、Sidecar、Provider Profile | 58、Provider Driver 维护 | P7-07 工程接入、凭证状态、fake-server 验收、真实 smoke 未执行和安全边界必读 |
...
| `60_P7回归验收与版本收口报告.md` | `CURRENT` | 54-59、工程回归、真实 Codex、fake 异常路径、P7 截图 | P7 发布门恢复 | 当前 P7 条件通过、Provider 发布阻塞、版本和 task-baseline 决策必读 |
...
用于核对路由/fallback。57 记录当前三家凭证缺失和真实 smoke 未执行的事实，不得以 Profile
配置或 fake-server 验收推断 Provider healthy。
